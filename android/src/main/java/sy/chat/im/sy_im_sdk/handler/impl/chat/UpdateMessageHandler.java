...
public class UpdateMessageHandler implements MethodHandler {
    @Override
    public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result, Context context) {
        if (call.hasArgument("syMessage")) {
            ...
            SyClient.getInstance().chatManager().updateMessage(syMessage, new SyCallback<SyMessage>() {
                @Override
                ...
                public void onFail(int i, String s) {
                    result.error(String.valueOf(i), s, "updateMessage error:" + s);
                }
            });
        } else {

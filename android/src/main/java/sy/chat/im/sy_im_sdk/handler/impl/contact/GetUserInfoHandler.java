...
public class GetUserInfoHandler implements MethodHandler {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result, Context context) {
        if (call.hasArgument("userId")) {
          ...
          SyClient.getInstance().contactManager().getUserInfo(userId, new SyCallback<SyContact>() {
              @Override
              public void onSuccess(SyContact syContact) {
                  result.success(GsonUtil.toJson(syContact));
              }
